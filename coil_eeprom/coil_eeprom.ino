...
void setup()
{
  ...
  for (int eeprom_entry_idx = 0; eeprom_entry_idx < 12; eeprom_entry_idx++) {
    ...
    for (int idx=0; idx<message.length(); idx++) {
      writeEEPROM(address, (byte) * (&(message[0]) + idx), EEPROM_I2C_ADDRESS);
      delay(5);
      address += 1;
    };
...
//      return;
  ...
  //pad 0 until 512 bytes
  for (int str_idx = 0; str_idx < (512-address); str_idx++) {
  ...
  for (int eeprom_entry_idx = 0; eeprom_entry_idx < 12; eeprom_entry_idx++) {
    ...
    for (int idx=0; idx<siemens_eeprom[eeprom_entry_idx].code_length; idx++) {
      ...
      if(readVal1==67) { cc_counter++; }; //two "C"?
      delay(5);
    };
    ...
    if (cc_counter==2) {
      ...
      Serial.println(String(coil_code, DEC));
    }
    else {
